...
SOURCE=Path('/Users/f0044gk/Desktop/SAXS/repository/BNL_SAXS/data/AA_052026.hdf5')
WINDOWS={'leading':(160,169),'center':(170,179),'trailing':(180,189),'late_tail':(190,199)}
...
def run():
 ...
 for baseline in ['linear','pre','post']:
  ...
  for lo,hi in WINDOWS.values():
   ...
   frac=np.clip((np.arange(lo,hi+1)-54.5)/(239.5-54.5),0,1).mean()
   if baseline=='pre':frac=0
   if baseline=='post':frac=1
   w[35:75]-=(1-frac)/40;w[225:255]-=frac/30;W.append(w)
 ...
 t=np.arange(145,216);f=(t-54.5)/185;traceY=Y[:,t]-Y[:,35:75].mean(1)[:,None]*(1-f)-Y[:,225:255].mean(1)[:,None]*f
 ...
 ax[0,0].set(xlabel='Frame (zero-based)',ylabel='Integrated scattering',title='AA direct averaging windows');ax[0,0].legend(fontsize=8)
 ...
 fig.tight_layout();fig.savefig(OUT/'AA_window_comparison.png',dpi=170)
 return summary
